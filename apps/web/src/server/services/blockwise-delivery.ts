...
export function blockwiseDeliveryRequest(payloadJson: string, eventId: string, timestamp: number | string = Math.floor(Date.now() / 1000), signature = signBlockwisePayload(payloadJson, timestamp)) {
