...
describe("calendar outbox ownership and lease recovery", () => {
  ...
  it("replays a Blockwise webhook with the exact persisted signed request", async () => {
    ...
    const event = buildBlockwiseBookingEvent({ id: bookingId, eventTypeId: eventType.id, workspaceId: eventType.workspaceId, blockwiseReference: "invite-outbox", inviteeName: "Outbox", inviteeEmail: "outbox@example.invalid", startAt: new Date("2099-01-02T00:00:00Z"), endAt: new Date("2099-01-02T00:30:00Z") }, "created", "blockwise-event-1", occurredAt);
    ...
    expect(calls[0]!.headers.get("x-snagtime-event-id")).toBe("blockwise-event-1"); expect(calls[1]!.headers.get("x-snagtime-event-id")).toBe("blockwise-event-1");
